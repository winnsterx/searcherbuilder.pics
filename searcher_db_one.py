...
def get_block_builder(block_number, w3, prefetched_blocks): 
...
def count_addrs_in_one_block(session, url, w3, prefetched_blocks, block_number):
    try: 
        global atomic_addrs
        builder = get_block_builder(block_number, w3, prefetched_blocks)
        payload = {
...
def count_addrs(start_block, num_blocks, prefetched_blocks):
    ...
    with requests.Session() as session:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            futures = [executor.submit(count_addrs_in_one_block, session, zeromev_url, w3, prefetched_blocks, b) for b in range(start_block, start_block + num_blocks)]
